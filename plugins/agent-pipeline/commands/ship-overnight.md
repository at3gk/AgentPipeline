...
**Model tier (automatic — no setup).** The **planner**, **coder**, and **reviewer** stages run on `claude-fable-5` when Fable is available and fall back to their defaults when it isn't. Detect availability from the first eligible delegation: spawn the **planner** with `model: claude-fable-5`. If that spawn **fails because Fable is unavailable or refused** (not accessible to this org, or a zero-data-retention `400`), re-run the planner on its default and use the defaults for the coder and reviewer this run; if it **succeeds**, use Fable for all three. The **Tester stays Sonnet**. **Keep the security-auditor on Opus regardless** — Fable's safety classifiers false-positive on security analysis. (Optional escape hatch: `AGENT_PIPELINE_FABLE=0` forces the default tier.) See `MODEL-TIERS.md`.
