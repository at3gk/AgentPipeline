...
1. **Determine the feature.**
   ...
   - Otherwise, if `$ARGUMENTS` is non-empty, that is the feature — with one exception: a bare `milestone:"<name>"` token is not a feature but a **restriction on the backlog pull** below (pull the next issue *from that milestone*).
   - If it is empty (or only a `milestone:"<name>"` restriction) **and** `docs/agents/issue-tracker.md` exists at the repo root (the repo tracks its backlog in GitHub Issues; labels are defined in `docs/agents/triage-labels.md`), pull from the tracker: run `gh issue list --label ready-for-agent --state open --json number,title,body,assignees,labels,createdAt` (add `--milestone "<name>"` when a milestone restriction was given), drop any issue that already has an assignee or an **open blocker** — a blocker is an open issue referenced either under a `## Blocked by` heading in the body or in an inline `Blocked by: #n` line (a `## Blocked by` section saying "None - can start immediately" is not a blocker); check each referenced issue's state, and only an issue that is still **open** blocks. Then pick: if `docs/agents/issue-tracker.md` defines **Priority** rules, order the remaining issues highest-priority-first — `P1` > `P2` > `P3` > unlabeled — breaking ties **oldest-first**; otherwise just take the **oldest**. Claim the pick with `gh issue edit <n> --add-assignee @me`, then fetch its full text with `gh issue view <n> --comments`. That is the feature; remember the number. **Never** pull `needs-triage` or `needs-info` issues — those are ungroomed candidates, not approved work. If no issue qualifies, write a short note saying there was nothing to do and stop.
   - If it is empty and there is no `docs/agents/issue-tracker.md`, read `FEATURES.md` at the repo root and take the first unchecked item (`- [ ] ...`) under the `## Ready to build` heading (if that heading exists; otherwise the first unchecked item anywhere). That is the feature. **Never** pull from a `## Proposed` section — those are ungroomed candidates, not approved work. If there is no backlog file or no unchecked ready item, write a short note saying there was nothing to do and stop. (Add one line to the morning report noting the repo has no issue-tracker conventions and that `/agent-pipeline:setup-pipeline`, run attended, would set them up.)
   - **Never install or configure anything mid-run.** You are unattended, and installing software is a hard-block class action. If `gh` is missing or unauthenticated, or the tracker setup turns out to be incomplete, do not fix it — fall back to the `FEATURES.md` behavior above and note the missing setup in the morning report.
...
3. **Do not open a pull request. Do not merge.** Leave the branch for my review. (When I later ask for the merge into main — usually in a fresh attended session — the merge flow is: `/map-repo` to refresh `REPO_CONTRACT.md` against the shipped changes, commit the refresh on the branch, full suite green, merge + push, then close the covered issues with a shipped comment. Note this in the morning report's next-steps so the merging session picks it up.)
