...
**Model tier.** Check `AGENT_PIPELINE_FABLE` once at the start. If it is `1`, delegate to the **planner**, **coder**, and **reviewer** stages with `model: claude-fable-5` (per-spawn override) for the highest-quality plan, implementation, and review on this unattended run; the Tester stays Sonnet. **Keep the security-auditor on Opus regardless** — Fable's safety classifiers false-positive on security analysis. If the variable is unset, delegate normally (Coder Sonnet, the rest Opus). See `MODEL-TIERS.md`.
